...
function cbtl_req(){
    if [ "$#" -lt 2 ]; then
        ...
        echo -e "\t e.g. cbtl_req 1 2"
        return
    fi
    command="./cbtl-request -p manager-$1.pub -s manager-$1 -a manager-$1.access -m master.pub -P patient-$2.pub -I"
    echo "${command}"
    eval $command
    return $?
}
...
function cbtl_req_rn(){
    ...
    do
        x=`shuf -i 0-$1 -n1`
        y=`shuf -i 0-$2 -n1`
        command="echo M$x""P$y.$i | cbtl_req $x $y"
        echo "${command}"
        eval $command
    done
    return $?
